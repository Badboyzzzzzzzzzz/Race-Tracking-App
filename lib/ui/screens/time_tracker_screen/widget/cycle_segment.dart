...
class _CycleSegmentScreenState extends State<CycleSegment> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      await segmentProvider.fetchSegmentResults('cycle');

      // Load race status
      await raceStatusProvider.loadSegmentStatus('cycle');
  ...
  void _handleBibTap(String bib) async {
    ...
    setState(() {
      ...
      if (preselectedBibs.contains(bib)) {
        ...
        segmentProvider.addResult(bib, 'cycle', _stopwatch.elapsed);
      } else {
        // First tap → preselect
          ...
          children: [
            Expanded(
              child: CustomButton(
                ...
                onPressed: () async {
                  if (!_stopwatch.isRunning) {
                    await raceStatusProvider.startSegment('cycle');
                    setState(() {
                      _stopwatch.start();
                    });
                  }
            ...
            Expanded(
              child: CustomButton(
                ...
                onPressed: () async {
                  await raceStatusProvider.completeSegment('cycle');
                  setState(() {
                    _stopwatch.stop();
                    _updateDisplay();
                  });
